...
class D
{
    ...
    public static function WipeAccount()
    {
        try {
            ...
            } elseif ((0 <= $_POST["gm"]) && ($_POST["gm"] <= 3)) { // Single mode
                if ($_POST["rx"] == 0) {
                    $modeInts = [$_POST["gm"]];
                } elseif ($_POST["rx"] == 1) {
                    if ($_POST["gm"] == 3) {
    ...
    public static function TakeReport()
    {
        try {
            ...
            } elseif ($status["assigned"] == $_SESSION["userid"]) {
                // Unassign
    ...
    public static function SaveClan()
    {
        try {
            ...
            // Update clan
            $GLOBALS['db']->execute(
                'UPDATE clans SET name = ?, tag = ?, description = ?, icon = ?, background = ?, status = ? WHERE id = ? LIMIT 1',
