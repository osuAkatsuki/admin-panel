...
class P
{
        ...
        */
        ...
        foreach ($users as $user) {
            ...
            if (($user["privileges"] & Privileges::UserPublic) == 0 && ($user["privileges"] & Privileges::UserNormal) == 0) {
                ...
                $allowedColor = "danger";
                $allowedText = "Banned";
            } elseif (($user["privileges"] & Privileges::UserPublic) == 0 && ($user["privileges"] & Privileges::UserNormal) > 0) {
    ...
    public static function AdminEditUser()
    {
        try {
            ...
            if (isBanned($userData["id"])) {
                echo "Banned";
            } elseif (isRestricted($userData["id"])) {
                echo "Restricted";
            } elseif (! hasPrivilege(Privileges::UserNormal, $userData["id"])) {
                echo "Locked";
            } else {
                echo "Ok";
            }
    ...
    public static function AdminViewReports()
    {
        ...
        foreach ($reports as $report) {
            if ($report['assigned'] == 0) {
                $rowClass = "danger";
                $assignee = "No one";
            } elseif ($report['assigned'] == -1) {
                $rowClass = "success";
                $assignee = "Solved";
            } elseif ($report["assigned"] == -2) {
                $rowClass = "warning";
                $assignee = "Useless";
            } else {
                $rowClass = "";
    ...
    public static function AdminSearchUserByIPResults()
    {
        try {
            ...
            } elseif (isset($_POST["ips"]) && ! empty($_POST["ips"])) {
                $ips = explode("\n", $_POST["ips"]);
            } else {
    ...
    public static function AdminClans()
    {
        try {
            ...
            if ($totalPages > 1) {
                ...
                $queryParams = [];
                if (! empty($searchName)) {
                    $queryParams[] = 'search_name=' . urlencode($searchName);
                }
                if (! empty($searchTag)) {
                    $queryParams[] = 'search_tag=' . urlencode($searchTag);
                }
                if (! empty($searchOwner)) {
                    $queryParams[] = 'search_owner=' . urlencode($searchOwner);
                }
